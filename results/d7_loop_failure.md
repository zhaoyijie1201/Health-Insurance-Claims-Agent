...
|---|---|---|---|---|---|---|---|
| working agent, sequential (before) | 91/91 | 4 | 10 | 0 | 1,027,287 | 0.1115 | 1.00x |
| minus the memory, de-duplication ON (caught) | 21/91 | 3 | 3 | {'duplicate_action': 70} | 594,932 | 0.0631 | 0.58x |
| minus the memory, de-duplication OFF (runaway) | 21/91 | 12 | 12 | {'step_cap': 70} | 2,771,782 | 0.2883 | 2.70x |
| working agent, parallel (shipped) | 91/91 | 2 | 4 | 0 | 717,316 | 0.0806 | 0.70x |

## 1 · The instrumentation that found it
Turns, model calls, tokens and cost are logged per run (agent.py). With de-duplication deleted, nothing raised an exception: 70 of 91 trials ran to the step cap of 12 turns and the set cost 2.70x the working agent's tokens. The runs that did not loop are the escalations that end before the policy row is needed twice.

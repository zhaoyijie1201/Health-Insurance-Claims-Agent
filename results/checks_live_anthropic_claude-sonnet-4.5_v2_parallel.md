# Checks used, case by case · live_anthropic/claude-sonnet-4.5_v2_parallel

backend live · model anthropic/claude-sonnet-4.5 · tools v2 · 2026-09-05 · 91 trials. CODE = harness.code_check against the answer key (decision, trigger, named line, approved_total, gated action once). JUDGEMENT = model: google/gemini-2.5-flash ruling on every must_record item with the prompt in docs/JUDGE_PROMPT.md.
...
| CLM-8960 | four_line_long_run | approve_in_principle | code + judgement | PASS 1/1 | decision, approved_total, gated_once | PASS 3/3 |  |
...
| CLM-9015 | two_preauth_one_expired | request_document | code + judgement | FAIL 1/3 | decision, missing_line, gated_once | PASS 3/3 |  |
...
| CLM-9018 | boundary_one_over_limit | escalate | code + judgement | PASS 3/3 | decision, trigger, gated_once | FAIL 2/3 | that lines were not individually priced |
...
**44/45 cases pass the code check on every trial; 38/45 pass the judgement check; 37/45 pass both.**

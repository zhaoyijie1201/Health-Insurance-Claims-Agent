# Checks used, case by case · live_mistralai/mistral-medium-3-5_v2_parallel

backend live · model mistralai/mistral-medium-3-5 · tools v2 · 2026-09-05 · 91 trials. CODE = harness.code_check against the answer key (decision, trigger, named line, approved_total, gated action once). JUDGEMENT = model: google/gemini-2.5-flash ruling on every must_record item with the prompt in docs/JUDGE_PROMPT.md.
...
| CLM-9010 | five_lines_no_preauth | approve_in_principle | code + judgement | PASS 1/1 | decision, approved_total, gated_once | FAIL 2/3 | itemised bill present for 45378 |
...
| CLM-9018 | boundary_one_over_limit | escalate | code + judgement | FAIL 2/3 | decision, trigger, gated_once | FAIL 2/3 | that lines were not individually priced |
...
| CLM-9021 | limit_exceeded_second_policy | escalate | code + judgement | PASS 3/3 | decision, trigger, gated_once | FAIL 2/3 | that lines were not individually priced |
...
| CLM-9029 | outside_policy_dates_after_end | escalate | code + judgement | PASS 3/3 | decision, trigger, gated_once | FAIL 2/3 | status active is not enough |
...
**43/45 cases pass the code check on every trial; 28/45 pass the judgement check; 28/45 pass both.**

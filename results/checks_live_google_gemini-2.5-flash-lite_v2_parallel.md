# Checks used, case by case · live_google/gemini-2.5-flash-lite_v2_parallel

backend live · model google/gemini-2.5-flash-lite · tools v2 · 2026-09-05 · 91 trials. CODE = harness.code_check against the answer key (decision, trigger, named line, approved_total, gated action once). JUDGEMENT = model: openai/gpt-4.1-mini ruling on every must_record item with the prompt in docs/JUDGE_PROMPT.md.
...
| CLM-8910 | policy_lapsed | escalate | code + judgement | FAIL 0/3 | decision, trigger, gated_once | PASS 2/2 |  |
...
| CLM-9008 | near_miss_duplicate_extra_line | approve_in_principle | code + judgement | PASS 1/1 | decision, approved_total, gated_once | FAIL 1/2 | NOT a duplicate of CLM-8710: same member, hospital and date, but the lines differ (an extra 99213) |
...
| CLM-9017 | two_preauth_both_absent | request_document | code + judgement | PASS 3/3 | decision, missing_line, gated_once | FAIL 1/2 | that nothing was found for either line (found is empty, not expired) |
...
| CLM-9020 | second_lapsed_policy | escalate | code + judgement | FAIL 0/3 | decision, trigger, gated_once | PASS 2/2 |  |
...
| CLM-9029 | outside_policy_dates_after_end | escalate | code + judgement | FAIL 0/3 | decision, trigger, gated_once | PASS 3/3 |  |
...
**38/45 cases pass the code check on every trial; 27/45 pass the judgement check; 23/45 pass both.**

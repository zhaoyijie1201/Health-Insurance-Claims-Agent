...
| credulous | v1 | whole set | 70/82 | only the hostile-text cases fail |
| credulous | v2 | whole set | 82/82 | nothing else moved |
| careful | v1 | whole set | 70/82 | even the careful agent fails the hostile cases on v1: without narrative_flags it prices the line and approves-with-refusal instead of escalating |
...
Cost of the fix: the v2 tool block is 1450 tokens against 595 for v1, re-sent every turn (D6 lever 1), and get_claim returns ~101.2 tokens against ~72.9 (lever 3). That is the price of a refusal that cannot be talked out of firing.
